...
import { FeeBreakdownDisplay, ChannelDurationSelector } from '../../components/ChannelConfiguration'
...
export const Step2: React.FC<Props> = ({ onNext, onBack }) => {
  ...
  const [effectiveMinCapacity, setEffectiveMinCapacity] = useState<number>(MIN_CHANNEL_CAPACITY)
  const [effectiveMaxCapacity, setEffectiveMaxCapacity] = useState<number>(MAX_CHANNEL_CAPACITY)
  const [fees, setFees] = useState<ChannelFees | null>(null)
  const [isLoadingFees, setIsLoadingFees] = useState(false)

  const { handleSubmit, setValue, control, watch, formState } = useForm<FormFields>({
    defaultValues: {
      assetId: '',
      capacitySat: '100000',
  ...
  const assetPresetsCalc = assetMax > 0
    ...
    : []
  const isCustomAssetTotal = assetPresetsCalc.length > 0 && lspAsset > 0 &&
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (infoResponse.data) {
          const sanitizedOptions: LspOptions = {
            ...
            min_onchain_payment_confirmations:
              infoResponse.data.options.min_onchain_payment_confirmations ?? undefined,
            min_onchain_payment_size_sat:
              infoResponse.data.options.min_onchain_payment_size_sat || 0,
          }
          setLspOptions(sanitizedOptions)

          const lspMinCapacity = infoResponse.data.options.min_channel_balance_sat || 0
          const lspMaxCapacity =
            infoResponse.data.options.max_channel_balance_sat || Number.MAX_SAFE_INTEGER

          setEffectiveMinCapacity(Math.max(MIN_CHANNEL_CAPACITY, lspMinCapacity))
          setEffectiveMaxCapacity(Math.min(MAX_CHANNEL_CAPACITY, lspMaxCapacity))

          if (infoResponse.data.assets && Array.isArray(infoResponse.data.assets)) {
        ...
        if (
          ...
          (error?.error && typeof error.error === 'string' && error.error.includes('timeout'))
        ) {
  ...
  const onSubmit = useCallback(
    (data: FormFields) => {
      if (!data.capacitySat) {
        toast.error(t('orderChannel.step2.capacityRequired'), { autoClose: 5000, position: 'bottom-right' })
        return
      }
      if (!data.clientBalanceSat) {
        toast.error(t('orderChannel.step2.liquidityRequired'), { autoClose: 5000, position: 'bottom-right' })
        return
      }
      if (addAsset && !data.assetId) {
        toast.error(t('orderChannel.step2.assetRequired'), { autoClose: 5000, position: 'bottom-right' })
        return
      }
      if (addAsset && data.assetId && !assetMap[data.assetId]) {
        toast.error(t('orderChannel.step2.assetUnsupported'), { autoClose: 5000, position: 'bottom-right' })
        return
      }
      if (addAsset && data.assetId && (data.lspAssetAmount === '' || data.lspAssetAmount === '0')) {
        toast.error(t('orderChannel.step2.assetAmountRequired'), { autoClose: 5000, position: 'bottom-right' })
        return
      }
      ...
      const parsedCapacitySat = parseInt(finalCapacitySat.replace(/[^0-9]/g, ''), 10)
      if (isNaN(parsedCapacitySat) || parsedCapacitySat < effectiveMinCapacity) {
        finalCapacitySat = effectiveMinCapacity.toString()
        setValue('capacitySat', finalCapacitySat)
        toast.info(
          t('orderChannel.step2.capacityAdjusted', { amount: formatNumberWithCommas(finalCapacitySat) }),
          { autoClose: 3000, position: 'bottom-right' }
        )
      }

      const parsedClientBalanceSat = parseInt(finalClientBalanceSat.replace(/[^0-9]/g, ''), 10)
      const minClientBalance = lspOptions?.min_initial_client_balance_sat || 0
      if (isNaN(parsedClientBalanceSat) || parsedClientBalanceSat < minClientBalance) {
        finalClientBalanceSat = minClientBalance.toString()
        setValue('clientBalanceSat', finalClientBalanceSat)
        toast.info(
          t('orderChannel.step2.liquidityAdjusted', { amount: formatNumberWithCommas(finalClientBalanceSat) }),
          { autoClose: 3000, position: 'bottom-right' }
        )
      }
      ...
      if (addAsset && data.assetId) {
        ...
        if (info) {
          const minAssetAmount = info.min_channel_amount / Math.pow(10, info.precision)
          const maxAssetAmount = info.max_channel_amount / Math.pow(10, info.precision)
          const parsedAmount = parseFloat(data.lspAssetAmount || '0')

          if ((parsedAmount !== 0 && isNaN(parsedAmount)) || (parsedAmount > 0 && parsedAmount < minAssetAmount)) {
            setValue('lspAssetAmount', minAssetAmount.toString())
            toast.info(
              t('orderChannel.step2.assetAdjusted', { amount: formatNumberWithCommas(minAssetAmount.toString()), ticker: info.ticker }),
              { autoClose: 3000, position: 'bottom-right' }
            )
          }
          if (parsedAmount > maxAssetAmount) {
            toast.error(
              t('orderChannel.step2.assetExceedsMax', { amount: formatNumberWithCommas(maxAssetAmount.toString()), ticker: info.ticker }),
              { autoClose: 5000, position: 'bottom-right' }
            )
            return
          }
        }
        parsedLspAssetAmount = parseAssetAmount(data.lspAssetAmount || '0', data.assetId)
        const parsedClientAsset = parseAssetAmount(data.clientAssetAmount || '0', data.assetId)
        parsedLspAssetAmount = Math.max(0, parsedLspAssetAmount - parsedClientAsset)
      }
      ...
      const submissionData: TChannelRequestForm = {
        ...
        clientBalanceSat: parseInt(finalClientBalanceSat.replace(/[^0-9]/g, ''), 10),
      ...
      try {
        ...
        if (error instanceof z.ZodError) {
          ...
          error.errors.forEach((err) => {
            ...
            if (path === 'capacitySat' && err.message.includes('minimum')) {
            ...
            } else if (path === 'capacitySat' && err.message.includes('maximum')) {
              adjustedData.capacitySat = MAX_CHANNEL_CAPACITY
              setValue('capacitySat', MAX_CHANNEL_CAPACITY.toString())
              madeAdjustments = true
            } else if (path === 'clientBalanceSat' && err.message.includes('minimum')) {
              ...
              madeAdjustments = true
            } else if (path === 'lspAssetAmount' && addAsset && data.assetId && assetMap[data.assetId]) {
              const info = assetMap[data.assetId]
              const minAssetAmount = info.min_channel_amount / Math.pow(10, info.precision)
              adjustedData.lspAssetAmount = minAssetAmount * Math.pow(10, info.precision)
              setValue('lspAssetAmount', minAssetAmount.toString())
              madeAdjustments = true
            }
          })
          ...
          if (madeAdjustments) {
            toast.info(t('orderChannel.step2.adjustmentsMade'), { autoClose: 3000, position: 'bottom-right' })
            dispatch(orderChannelSliceActions.setChannelRequestForm(adjustedData))
            onNext(adjustedData)
            return
          }
    ...
    [addAsset, onNext, parseAssetAmount, dispatch, setValue, effectiveMinCapacity, lspOptions, assetMap]
  )
  ...
  useEffect(() => {
    const fetchFees = async () => {
      ...
      const parsedClientBalance = parseInt(clientBalanceSat.replace(/[^0-9]/g, ''), 10)
      ...
      try {
        ...
        if (addAsset && assetId && lspAssetAmount && assetMap[assetId]) {
          ...
          const parsedTotal = parseAssetAmount(lspAssetAmount, assetId)
          const parsedClient = parseAssetAmount(clientAssetAmount || '0', assetId)
    ...
    <div className="w-full">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-content-secondary mt-2">{t('orderChannel.step2.subtitle')}</p>
        </div>
        ...
        <form className="w-full space-y-4" onSubmit={handleSubmit(onSubmit)}>
          {isLoading ? (
            ...
            <>
              ...
              {/* BTC Capacity section */}
              <div className="rounded-xl border border-border-subtle p-4 space-y-4 bg-surface-overlay/30">
                <div className="flex items-center gap-3">
                  <img src={bitcoinLogo} alt="BTC" className="w-6 h-6" />
                  <div>
                    <p className="font-semibold text-sm text-content-primary">BTC Lightning Channel</p>
                    <p className="text-xs text-content-secondary">
                      Total: {currentCapacity.toLocaleString()} sats
                    </p>
                  </div>
                </div>
                    ...
                    {filteredPresets.map((preset) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (btcOut > preset) {
                            setValue('clientBalanceSat', Math.floor(preset / 2).toString())
                          }
                        }}
                      >
                  ...
                  {(isCustomCapacity || showCustomInput) && (
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (btcOut > newCap) {
                          setValue('clientBalanceSat', Math.floor(newCap / 2).toString())
                        }
                      }}
                      min={effectiveMinCapacity}
                      max={effectiveMaxCapacity}
                      placeholder="Custom capacity (sats)"
                      className="mt-2 w-full px-3 py-2 bg-surface-overlay rounded-xl border border-border-default focus:border-amber-400 text-white text-sm outline-none"
                    />
                  )}
                </div>
                ...
                <LiquidityBar
                  outbound={btcOut}
                  inbound={btcIn}
                  outboundLabel={`${btcOut.toLocaleString()} sats`}
                  inboundLabel={`${btcIn.toLocaleString()} sats`}
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <input
                        ...
                        max={Math.min(currentCapacity, lspOptions?.max_initial_client_balance_sat || currentCapacity)}
                        step={1000}
                        value={btcOut}
                        onChange={(e) => {
                          const val = parseInt(e.target.value, 10)
                          if (!isNaN(val)) setValue('clientBalanceSat', val.toString())
                        }}
                        className="w-28 px-2 py-0.5 bg-surface-overlay rounded-lg border border-border-default focus:border-amber-400 text-amber-400 font-semibold text-xs outline-none text-right"
                      />
                      <span className="text-content-tertiary">sats</span>
                    </div>
                  </div>
                  <input
                    type="range"
                    ...
                    step={1000}
                    value={btcOut}
                    onChange={(e) => setValue('clientBalanceSat', e.target.value)}
                  ...
                  <div className="flex justify-between text-[10px] text-content-tertiary mt-1">
                    <span>
                      {(lspOptions?.min_initial_client_balance_sat || 0).toLocaleString()} sats min
                    </span>
                    <span>
                      {Math.min(
                        currentCapacity,
              ...
              {/* RGB Asset section */}
              {addAsset && (
                <div className="rounded-xl border border-purple-400/20 p-4 space-y-4 bg-surface-overlay/30">
                  <div className="flex items-center gap-2">
                    <img src={rgbIcon} alt="RGB" className="w-6 h-6" />
                    <div>
                      <p className="font-semibold text-sm text-content-primary">RGB Asset Channel</p>
                      <p className="text-xs text-content-secondary">
                        Select an asset and its inbound capacity
                      </p>
                    </div>
                  </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {assetPresetsCalc.map((preset) => (
                            <button
                              ...
                              type="button"
                              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                                Math.abs(lspAsset - preset) < 0.001 && !showCustomAssetCapacity
                                  ? 'bg-purple-400/20 text-purple-400 border border-purple-400/50'
                                  : 'bg-surface-overlay text-content-secondary border border-border-subtle hover:border-purple-400/30 hover:text-content-primary'
                              }`}
                              onClick={() => {
                                setShowCustomAssetCapacity(false)
                                setValue('lspAssetAmount', preset.toString())
                                if (clientAsset > preset) setValue('clientAssetAmount', preset.toString())
                              }}
                            >
                              {preset >= 1000
                                ? `${(preset / 1000).toFixed(1)}K`
                                : preset.toFixed(assetInfo.precision > 0 ? 2 : 0)}{' '}
                              {assetInfo.ticker}
                            </button>
                          ))}
                          <button
                        ...
                        {(showCustomAssetCapacity || isCustomAssetTotal) && (
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (clientAsset > newTotal) setValue('clientAssetAmount', e.target.value)
                            }}
                            min={0}
                            max={assetMax}
                            step={1 / assetFactor}
                            placeholder={`Custom (${assetInfo.ticker})`}
                            className="mt-2 w-full px-3 py-2 bg-surface-overlay rounded-xl border border-border-default focus:border-purple-400 text-white text-sm outline-none"
                          />
                        )}
                      </div>

                      {/* Liquidity bar: outbound = clientAsset, inbound = lspAsset - clientAsset */}
                      <LiquidityBar
                        outbound={clientAsset}
                        inbound={Math.max(0, lspAsset - clientAsset)}
                        ...
                        outboundColor="bg-purple-400"
                        inboundColor="bg-purple-400/30"
                      />

                      {/* Buy amount slider */}
                      <div>
                        <div className="flex items-center justify-between text-xs mb-1.5">
                          <span className="text-content-secondary">Your outbound (buy amount)</span>
                          <div className="flex items-center gap-1.5">
                            <input
                              type="number"
                              min={0}
                              ...
                              onChange={(e) => {
                                ...
                                if (!isNaN(val)) setValue('clientAssetAmount', val.toString())
                              }}
                              className="w-24 px-2 py-0.5 bg-surface-overlay rounded-lg border border-border-default focus:border-purple-400 text-purple-400 font-semibold text-xs outline-none text-right"
                            />
                            <span className="text-content-tertiary">{assetInfo.ticker}</span>
                          </div>
                        </div>
                        <input
                          type="range"
                        ...
                        <div className="flex justify-between text-[10px] text-content-tertiary mt-1">
                          ...
                          <span>
                            {(lspAsset || assetMax).toFixed(assetInfo.precision > 0 ? 2 : 0)}{' '}
                            {assetInfo.ticker} (all outbound)
                          </span>
                        </div>
                      </div>
                    </>
