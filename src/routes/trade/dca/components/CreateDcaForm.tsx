...
interface UsdtLnBalance {
  balance: number | undefined
  hasReadyUsdtChannel: boolean | undefined
}

function useUsdtLnBalance(): UsdtLnBalance {
  ...
  if (!channelsData?.channels || !assetsData?.nia)
    return { balance: undefined, hasReadyUsdtChannel: undefined }

  const usdtAsset = assetsData.nia.find((a: any) => a.ticker === 'USDT')
  if (!usdtAsset) return { balance: 0, hasReadyUsdtChannel: false }
  ...
  const usdtChannels = channelsData.channels.filter(
    (ch: any) => ch.asset_id === usdtAssetId && ch.ready
  )

  return {
    balance: usdtChannels.reduce(
      (sum: number, ch: any) =>
        sum + (ch.asset_local_amount ?? 0) / precisionFactor,
      0
    ),
    hasReadyUsdtChannel: usdtChannels.length > 0,
  }
}
...
export function CreateDcaForm({ currentBtcPrice, onCreated }: Props) {
  ...
  const { balance: usdtBalance, hasReadyUsdtChannel } = useUsdtLnBalance()
  ...
  const noUsdtChannel = hasReadyUsdtChannel === false
      ...
      <div className="flex flex-col items-center gap-6 py-4 text-center">
        ...
        <div className="space-y-2">
          <h4 className="text-base font-semibold text-white">
            {t('dca.noChannel.title', 'No USDT Lightning channel')}
          </h4>
          <p className="text-sm text-content-secondary leading-relaxed max-w-xs">
            {t(
              'dca.noChannel.description',
              'You need a USDT Lightning channel to run DCA orders. Buy one to get started.'
            )}
          </p>
        </div>
        <button
        ...
        >
          ...
          {t('dca.noChannel.cta', 'Buy USDT Channel')}
        </button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-5">
        ...
        <button
          className="w-full py-2.5 rounded-lg bg-primary text-white font-semibold text-sm
