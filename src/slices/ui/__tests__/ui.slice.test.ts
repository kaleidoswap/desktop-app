...
describe('setModal', () => {
  it('sets a deposit modal', () => {
    const state = reducer(initialState, uiSliceActions.setModal({ type: 'deposit', assetId: 'rgb:abc' }))
    expect(state.modal).toEqual({ type: 'deposit', assetId: 'rgb:abc' })
  })

  it('sets a withdraw modal', () => {
    const state = reducer(initialState, uiSliceActions.setModal({ type: 'withdraw', assetId: undefined }))
...
describe('uiSliceSeletors.modal', () => {
  it('returns the current modal from state', () => {
    const rootState = { ui: { modal: { type: 'deposit' as const, assetId: 'rgb:xyz' } } }
    expect(uiSliceSeletors.modal(rootState)).toEqual({ type: 'deposit', assetId: 'rgb:xyz' })
  })
