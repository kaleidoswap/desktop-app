import { XCircle, Shield, CheckCircle2, Loader2, RefreshCw, Globe, Zap, ArrowLeft } from 'lucide-react'
  ...
  return (
    <div className="w-full max-w-md mx-auto flex flex-col space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col items-center text-center space-y-3">
        <div
          className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-2 shadow-lg ${errorMessage
      ...
      {errorMessage && (
        <div className="p-4 rounded-xl bg-status-danger/10 border border-status-danger/30 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h4 className="font-semibold text-status-danger text-sm mb-1">Error Details</h4>
        ...
        <div className="space-y-2">
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border transition-colors ${isCompleted
      ...
      {(onBack || onCancel) && (
        <div className="pt-4 flex flex-col gap-3">
          {onCancel && (
            <Button
              className="w-full"
              icon={errorMessage ? <RefreshCw className="w-4 h-4" /> : <XCircle className="w-4 h-4" />}
              onClick={onCancel}
              size="lg"
